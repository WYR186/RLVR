...
def timeout(timeout_seconds: int = 8):
    if os.name == "posix":
        ...
        # Reasoning360's released verifier uses SIGALRM and therefore fails
        # at import time on Windows. The 4070 variant keeps the scoring logic
        # byte-for-byte otherwise and uses guarded inputs plus the verifier's
        # existing exception handling instead of a POSIX alarm.
        def decorator(func):
            return func
