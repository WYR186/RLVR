...
| Phase 2 v3 ckpt-0 comparability gate | RTX 4070 Laptop (CUDA bf16 measurement) | hard stop for Phase 3 | `metrics_ckpt0.json` written; Gate `ckpt0` returned STOP because effective-rank deltas versus the committed pilot were layer4 0.3910, layer12 0.2936, and layer22 0.0305, exceeding the 0.01 tolerance; Phase 2 measurements are retained as evidence, but Phase 3 adaptation will not be started under the pre-registered gate |
| Phase 2 v3 measurement | RTX 4070 Laptop (CUDA bf16 measurement) | 1.4 min | all eight checkpoint metric files (`0,25,50,100,200,300,400,500`) and `phase2_complete.json` saved; GPU telemetry and launcher/wrapper transcripts copied under `outputs/exp15_cuda_grpo_gsm8k_c7cc7a1d02d9/telemetry/`; Phase 3 deliberately not launched because the recorded ckpt-0 comparability gate was STOP |
